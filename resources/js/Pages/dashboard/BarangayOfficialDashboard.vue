...
const barangayName = computed(() => userBarangay.value?.name || 'Loading...');
const municipalityName = computed(() => userBarangay.value?.municipality?.name || 'Loading...');
const locationString = computed(() => `Brgy. ${barangayName.value}, ${municipalityName.value}`);
...
// Add this function to fetch barangay data
const fetchBarangayData = async () => {
  const { data: barangay } = await supabase
    .from('barangays')
    .select('*, municipality:municipality_id(*)')
    .eq('id', user.value.barangay_id)
    .single();

  if (barangay) {
    userBarangay.value = barangay;
  }
};

// Fetch all data
const fetchData = async () => {
  // Fetch stranding stats for user's barangay
  const { data: strandings } = await supabase
    .from('stranded_incidents')
    .select('*')
    .eq('is_active', true)
    .eq('barangay_id', userBarangay.value.id);
  ...
  // Fetch pending sightings for user's barangay
  ...
  // Fetch completed reports for user's barangay
...
<template>
  <Head title="Barangay Official Dashboard" />
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div class="mb-6">
            ...
            <p class="text-white text-opacity-80">
              Barangay Official - {{ locationString }} Monitoring Dashboard
            </p>
          </div>

          <!-- Stats Cards using glass-container -->
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-3 mb-6">
            <!-- Unresolved Strandings in Barangay -->
            <div class="glass-container overflow-hidden rounded-lg shadow">
              <div class="p-5">
                <div class="flex items-center">
                  ...
                  <div class="ml-5 w-0 flex-1">
                    <dl>
                      <dt class="text-sm font-medium text-gray-500 truncate">Active Cases in {{ locationString }}</dt>
                      <dd>
            ...
            <div class="glass-container overflow-hidden rounded-lg shadow">
              <div class="p-5">
                <div class="flex items-center">
                  ...
                  <div class="ml-5 w-0 flex-1">
                    <dl>
                      <dt class="text-sm font-medium text-gray-500 truncate">Pending Sightings in {{ locationString }}</dt>
                      <dd>
            ...
            <div class="glass-container overflow-hidden rounded-lg shadow">
              <div class="p-5">
                <div class="flex items-center">
                  ...
                  <div class="ml-5 w-0 flex-1">
                    <dl>
                      <dt class="text-sm font-medium text-gray-500 truncate">Completed Reports in {{ locationString }}</dt>
                      <dd>
          ...
          <!-- Rest of the content with glass-container updates -->
          <!-- Map and Alerts Section -->
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <!-- Map Section - Takes up 2/3 of the width on large screens -->
            <div class="glass-container rounded-lg shadow lg:col-span-2">
              <div class="px-6 py-5 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">Active Cases in {{ locationString }}</h3>
              </div>
              <div class="p-6">
            ...
            <div class="glass-container rounded-lg shadow">
              <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg font-medium text-gray-900">Urgent Cases in {{ locationString }}</h3>
                <Link
          ...
          <div class="glass-container rounded-lg shadow mb-6">
            <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-center">
              <h3 class="text-lg font-medium text-gray-900">Active Reports in {{ locationString }}</h3>
