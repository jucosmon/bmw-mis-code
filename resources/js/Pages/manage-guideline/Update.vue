...
const form = useForm({
    ...
    items: (props.guideline.items || []).map((item) => ({
        id: item.id, // Ensure this is set correctly
        count: item.count, // Use the count from the item if available
        text: item.text || '',
...
const removeImage = (itemIndex, imageIndex) => {
    const mediaFile = form.items[itemIndex].mediaFiles[imageIndex];
    if (mediaFile.id) {
...
const itemsChanged = computed(() => {
    return form.items.some((item, index) => {
        const originalItem = (props.guideline.items || [])[index] || {};
        return (
            item.guideline_id !== originalItem.guideline_id ||
            item.text !== originalItem.text ||
            (originalItem.mediaFiles ? item.mediaFiles.length !== originalItem.mediaFiles.length : item.mediaFiles.length > 0)
        );
    });
});
...
const hasChanges = computed(() => {
    ...
    const newItemsAdded = form.items.length > props.guideline.items.length;
            ...
            <div v-for="(item, index) in form.items" :key="index" class="sm:col-span-2 p-5 rounded-lg shadow-md">
              ...
              <div v-if="item.previewFiles.length">
                ...
                <div class="flex space-x-2">
                  ...
                  <button @click.prevent="removeImage(index, imgIndex)" class="text-red-500">Remove</button>
                </div>
              </div>
